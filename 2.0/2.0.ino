...
int repetitionCounts[4] = {1, 1, 1, 1};            // Default to 1 repetition
int currentRepetition[4] = {0, 0, 0, 0};           // Tracks repetitions completed
...
void handleIndividualMotors(unsigned long currentTime) {
  for (int i = 0; i < 4; i++) {
    ...
    if (!motorRunning[i] && currentTime >= nextRunTimes[i] && nextRunTimes[i] > 0) {
      if (currentRepetition[i] < repetitionCounts[i]) {
        motorRunning[i] = true;
        startTimes[i] = currentTime;
        Serial.print("Motor ");
        Serial.print((char)('X' + i));
        Serial.print(" started. Repetition ");
        Serial.print(currentRepetition[i] + 1);
        Serial.print(" of ");
        Serial.println(repetitionCounts[i]);
      }
    }
    ...
    if (motorRunning[i]) {
      ...
      if (elapsedTime >= rotationTimes[i]) {
        ...
        if (currentRepetition[i] < repetitionCounts[i]) {
          ...
          nextRunTimes[i] = 0; // Stop further repetitions
        }
        Serial.print("Motor ");
        Serial.print((char)('X' + i));
...
void processIndividualCommand(String command) {
  if (command.startsWith("START")) {
    ...
    if (motorIndex >= 0) {
      ...
      currentRepetition[motorIndex] = 0; // Reset repetition count
      Serial.print("Motor ");
  ...
  } else if (command.startsWith("REPEAT MOTOR")) {
    int motorIndex = getMotorIndex(command.charAt(13));
    int repeatValue = command.substring(15).toInt();
    if (motorIndex >= 0 && repeatValue > 0 && repeatValue <= 100) {
      repetitionCounts[motorIndex] = repeatValue;
      Serial.print("Repetition count for Motor ");
      Serial.print((char)('X' + motorIndex));
      Serial.print(" set to ");
